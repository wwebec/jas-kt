...
The Job-Room Jobs API (link:{api-url}/jobAdvertisements/v1[{api-url}/jobAdvertisements/v1]) provides an additional channel for employers to publish job advertisements to the Swiss public employment services system.
...
The legacy API (described at {api-url}/api-docs/joboffers/index.html) will be decommissioned at the same time.

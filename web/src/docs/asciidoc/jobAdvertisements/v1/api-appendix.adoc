...
=== Populate a database with Candidate data

To populate a local database with a candidate data, run the undermentioned *curl* script against REST APIs, under the ```/api``` URL with a corresponding payload.
e.g.
```
...
```
The payload scripts can be found under ```/src/curl```. Be aware of already existing candidate data and if there is such a need remove the old one.